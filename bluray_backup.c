...
int add_filename_to_tmpfile(char *filename, int fd) {
	...
	// FIXME this variable backup_mkdir_path could definitely use a better name
	...
	int log_filenames(BLURAY *bd, char *parent_dir, char *target_dir) {
		...
		while(bdnd_read == 0) {
			...
			if (strchr(bdnd_dirent->d_name, '.') == NULL) {
				...
				// FIXME should I add logic to react to this? Doesn't make much sense since it's recursive and nothing's going to change if it fails
				...
				// FIXME doing the counting here first, need this REGARDLESS of whether the actual backup of the file works or not
				bluray_files++;
	...
	int fd;
		...
		char mkfile_path[BLURAY_FILENAME_STRLEN];
			...
			while(parent_read == 0) {
				...
				// I haven't seen any files in root directories yet
				if(!directory) {
					...
					// Dump to stdout while I'm testing this
					printf("adding %s to filename tmpfile\n", mkfile_path);
